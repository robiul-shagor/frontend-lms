...
import { displayLeaseProperties, fetchMediaPropertyData } from "@/services/api";
...
export default async function NewListing() {
  ...
  try {
    ...
    const response = await displayLeaseProperties(); // Ensure this function returns a valid fetch response
    posts = await response; // Parse the JSON data

    const listingKeys = posts.map((property: any) => property.ListingKey);
    const mediaMap = await fetchMediaPropertyData(listingKeys);
    
    // Enrich properties with media
    const enrichedProperties = posts.map((property: any) => ({
      ...property,
    ...
    console.error('Failed to fetch lease property data:', error);
